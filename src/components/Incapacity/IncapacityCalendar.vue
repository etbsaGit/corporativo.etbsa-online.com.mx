...
const incapacities = ref([]);

const transformedVacations = computed(() =>
  incapacities.value.map((vacation) => ({
...
const getIncapacities = async (date) => {
  let res = await sendRequest(
    "GET",
    null,
    "/api/incapacity/calendar/" + date,
    ""
  );
  incapacities.value = res;
};

onMounted(() => {
  getIncapacities(selectedDate.value);
});
</script>
