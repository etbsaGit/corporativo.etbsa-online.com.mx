    ...
    <q-list bordered separator>
      <q-item v-for="pregunta in survey.question" :key="pregunta.id">
        ...
        <q-item-section>
          ...
          <q-input v-if="pregunta.type === 'text'" v-model="pregunta.respuesta" disable outlined dense
            placeholder="Sin respuesta aun">
          </q-input>
          <div v-if="pregunta.type === 'select'">
            <q-select v-model="pregunta.respuesta" :options="pregunta.data.map(item => item.data)" disable outlined dense>
          ...
          <q-checkbox v-if="pregunta.respuestaAsignada" v-model="pregunta.rating" :true-value="1" :false-value="0"
            checked-icon="task_alt" unchecked-icon="highlight_off" :color="getColor(pregunta.rating)">
