...
const getEstatus = async () => {
  let res = await sendRequest("POST", null, "/api/estatus", "");
  status.value = res;
};
...
const uploadFile = async () => {
  ...
  try {
    let res = await sendRequest("POST", formData, "/api/archivo", "");
  } catch (error) {
...
const getArchivos = async () => {
  let res = await sendRequest("POST", formRequisito, "/api/archivo", "");
  console.log(res);
};
