<template>
  ...
  >
    <q-card>
      ...
      <q-item>
        <q-item-section>
          <sucursal-form ref="edit" :sucursal="selectedRow" />
        </q-item-section>
      </q-item>
    </q-card>
  </q-dialog>
...
const getRows = async (page = 1) => {
  ...
  let res = await sendRequest("POST", final, "/api/sucursales", "");
  rows.value = res.data;
...
const deleteRow = async () => {
  let res = await sendRequest(
    "DELETE",
    null,
    "/api/sucursal/" + selectedRow.value.id,
    ""
  );
