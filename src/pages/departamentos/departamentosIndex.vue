<template>
  ...
  >
    <q-card>
      ...
      <q-item>
        <q-item-section>
          <departamento-form ref="edit" :departamento="selectedRow" />
        </q-item-section>
      </q-item>
    </q-card>
  </q-dialog>
...
const getRows = async (page = 1) => {
  ...
  let res = await sendRequest("POST", final, "/api/departamentos", "");
  rows.value = res.data;
...
const deleteRow = async () => {
  let res = await sendRequest(
    "DELETE",
    null,
    "/api/departamento/" + selectedRow.value.id,
    ""
  );
